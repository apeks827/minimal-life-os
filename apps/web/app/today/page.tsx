...
export default async function Page() {
  const { supabase, user } = await requireReadyUser("/today");
  const serverState = supabase && user ? await loadServerLifeState(supabase) : undefined;
  return <SectionPage title="План на сегодня" description="Фокус дня с учетом лимита задач, привычек и событий." {...(serverState ? { serverState } : {})} />;
}
