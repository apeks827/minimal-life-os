...
export default async function Page() {
  const { supabase, user } = await requireReadyUser("/tasks");
  const serverState = supabase && user ? await loadServerLifeState(supabase) : undefined;
  return <SectionPage title="Задачи" description="Все созданные задачи из единого входящего." {...(serverState ? { serverState } : {})} />;
}
